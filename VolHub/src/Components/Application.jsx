...
function Application() {
  ...
  return (
    <>
      <Box className="w-full h-screen flex flex-col">
        {/* Top Section */}
        <Box className="w-full flex items-center justify-between p-7 bg-[#ffffff]" borderColor={'gray.400'}>
          <Text className="text-3xl font-bold text-[#4d2408]">Start Your Volunteering Now!</Text>
          {isVolunteer && (
            <Input
        ...
        {/* Content Section */}
        <Box className="w-full h-full p-4 overflow-auto bg-[#ffffff]">
          {/* Event Cards */}
            ...
            {filteredApplications.map((item, index) => (
              ...
              >
                ...
                <Box className="p-4">
                  ...
                  {isVolunteer && <Button className="w-full mt-2"  bg="#2980B9" color="white"onClick={() => onApplyClick(item)}>Apply</Button>}
      ...
      {/* Create Event Form */}
      {isApplication && (
        <Box className="bg-white fixed top-5 right-10 w-4/12 h-[660px] mt-6 rounded-[50px] shadow-lg p-6 overflow-y-auto z-50">
  ...
  <Box className="bg-white fixed top-1/4 left-1/4 w-5/12 h-auto mt-6 rounded-[20px] shadow-lg p-4 z-50">
    ...
    <Box className="relative z-10 bg-white p-4 rounded-lg shadow-lg">
      ...
      />
      <Text className="text-left text-xl font-bold mb-2 text-[#4d2408]">{applyitem.eventname}</Text>
      <Text mb-4 className="text-lg text-black.500">{applyitem.description}</Text>
